# filename: lrv_boxplot_all_microorganisms.py
import pandas as pd
import numpy as np
...
# Plot box for each microorganism
...
# X position is categorical: to assign x coordinate for scatter points, use categoryindex
x_categories = microorganisms_ordered
cat_to_x = {cat: i for i, cat in enumerate(x_categories)}

# Prepare scatter lists for detected and non-detected
scatter_detect_x = []
scatter_detect_y = []
scatter_nondetect_x = []
scatter_nondetect_y = []
...
for idx, row in df_pairs.iterrows():
    ...
    if row['Effluent_detected_flag'] == 'Non-detect':
        ...
        # unknown detection status; ignore for marker plotting
        pass
...
print(html_file)
print(png_file)
print("lrv_boxplot_all_microorganisms.py")