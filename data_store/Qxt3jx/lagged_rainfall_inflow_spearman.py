# filename: lagged_rainfall_inflow_spearman.py
import pandas as pd
import numpy as np
...
results = []
lags = range(0, 15)  # 0 to 14 inclusive

for dam in common_dams:
    for lag in lags:
        # rainfall lead effect means correlate rainfall[t] vs inflow[t+lag]
        # so align by shifting inflow backward by lag days
        rain_series = pivot_rainfall[dam]
        inflow_series = pivot_inflow[dam].shift(-lag)  # shift upward so inflow day t+lag aligns with rainfall day t
        
        combined = pd.DataFrame({'rainfall': rain_series, 'inflow': inflow_series}).dropna()
        n_pairs = combined.shape[0]
        if n_pairs == 0:
            rho = np.nan
        else:
            # Spearman correlation and p-value, extract rho only
            rho, _ = spearmanr(combined['rainfall'], combined['inflow'])
        results.append({'dam': dam, 'lag_days': lag, 'spearman_rho': rho, 'n_pairs': n_pairs})
...
# Save outputs
base_name = "lagged_rainfall_inflow_spearman"
